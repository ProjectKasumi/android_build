...
EOF
    ...
    local A=""
    local i
    for i in `cat $T/build/envsetup.sh $T/${CUSTOM_VENDOR_DIR}/build/envsetup.sh | sed -n "/^[[:blank:]]*function /s/function \([a-z_]*\).*/\1/p" | sort | uniq`; do
      A="$A $i"
    done
    echo $A
}
...
function build_build_var_cache()
{
    ...
    cached_vars=(`cat $T/build/envsetup.sh $T/${CUSTOM_VENDOR_DIR}/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`)
    cached_abs_vars=(`cat $T/build/envsetup.sh $T/${CUSTOM_VENDOR_DIR}/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_abs_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`)
...
function lunch()
{
    ...
    then
        # if we can't find a product, try to grab it off Kasumi-Devices
        T=$(gettop)
        cd $T > /dev/null
